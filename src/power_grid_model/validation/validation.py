...
def _process_power_sigma_and_p_q_sigma(
    ...
    if sensor in data:
        ...
        pq_sigma_provided = np.any(
            np.logical_not(np.logical_and(np.isnan(p_sigma), np.isnan(q_sigma))),
            axis=asym_axes,
        )
        sensor_data["power_sigma"][pq_sigma_provided] = np.sqrt(
            np.mean(
                p_sigma[pq_sigma_provided] * p_sigma[pq_sigma_provided]
                + q_sigma[pq_sigma_provided] * q_sigma[pq_sigma_provided],
                axis=asym_axes,
            )
        )
